...
export const Lootbox = ({
    ...
    const { stakingProgram, lootboxProgram, switchboardProgram } =
        useWorkspace();
    ...
    useEffect(() => {
        if (!walletAdapter.publicKey || !lootboxProgram || !stakingProgram)
            return;
    ...
    const checkUserAccount = async (
        ...
        try {
            ...
            const account = await lootboxProgram.account.userState.fetch(
                userStatePda
            );
            if (account) {
    ...
    const fetchLootboxPointer = async (
        ...
        try {
            ...
            const lootboxPointer =
                await lootboxProgram.account.lootboxPointer.fetch(
                    lootboxPointerPda
                );

            setAvailableLootbox(lootboxPointer.availableLootbox.toNumber());
            setMint(
                lootboxPointer.redeemable ? lootboxPointer.mint : undefined
            );
        } catch (error) {
            console.log(error);
